...
import { playAudio, playPhonics, getLastPlayedAudio, stopAudio } from "@/lib/audio";
...
const Learning = () => {
  ...
  // Navigation flag for controlled transitions
  ...
  /* FORCE IMAGE RESET ON WORD CHANGE */
  useEffect(() => {
    setImageSrc("");
  }, [currentWord?.id]);
  ...
  useEffect(() => {
    if (!pendingNext || navigationLocked) return;

    setNavigationLocked(true); // 🔒 lock

    // hard reset first
    setWordCompleted(false);
    setCurrentInput("");

    // next frame guarantees React commit
    requestAnimationFrame(() => {
      setCurrentIndex(i => (i + 1) % words.length);
      setPendingNext(false);
    });
  }, [pendingNext, navigationLocked, words.length]);

  /* UNLOCK NAVIGATION AFTER INDEX CHANGE */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Play word audio then first letter
    const playWordAndFirstLetter = async () => {
      try {
        // Play the word first
        if (currentWord.audio) {
          await playAudio(currentWord.audio);
          lastPlayedAudio = currentWord.audio;
        } else {
          await playPhonics(currentWord.word);
        }
        
        // Small pause
        await new Promise(resolve => setTimeout(resolve, 500));
        
        // Then play first letter
    ...
    playWordAndFirstLetter();

    // Start idle reminder timer (2-3 seconds)
    idleTimer = window.setTimeout(() => {
      if (lastPlayedAudio) {
        playAudio(lastPlayedAudio).catch(console.error);
      }
    }, 2500);
    ...
    if (lower.length < currentWord.word.length) {
      ...
      if (celebrationEnabled) {
        ...
        celebrationTimerRef.current = window.setTimeout(() => {
          ...
          setStars([]);
          setPendingNext(prev => {
            if (navigationLocked) return prev;
            return true;
          });
        }, 5000);
      } else {
          ...
          <div className="flex-1 flex flex-col items-center gap-4 px-3 pt-4 pb-[280px]">
            ...
            <div className="w-full flex items-center justify-center gap-2 px-3">
              ...
              <div className="relative w-[80%] max-w-[352px]">
                {!categoryChanging && currentWord && imageSrc && (mode === "image-first" || wordCompleted) && (

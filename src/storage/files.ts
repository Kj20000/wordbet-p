...
export async function saveAudioFile(
  ...
  if (platform === "web") {
    const db = await imageDB;
    await db.put("audio", file, filename);
...
/* =========================
   LOAD AUDIO
========================= */
export async function loadAudioFile(filename: string): Promise<string> {
  const Capacitor = await getCapacitor();
  const platform = Capacitor?.getPlatform?.() ?? "web";
  ...
  if (platform === "web") {
    const db = await imageDB;
    const file = await db.get("audio", filename);
    if (!file) throw new Error("Audio not found");
    return URL.createObjectURL(file);
  }
...
export async function deleteAudioFile(filename: string) {
  ...
  if (platform === "web") {
    const db = await imageDB;
    await db.delete("audio", filename);

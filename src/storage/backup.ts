...
import { saveImageFile, loadImageFile, saveAudioFile, loadAudioFile, setAutoBackupFunction } from "./files";
import { 
...
interface BackupFile {
  ...
  audio?: Record<string, string>; // audioId -> base64 (WebM)
}
...
export async function exportBackup() {
  ...
  for (const w of words) {
    ...
    if (w.audio) {
      try {
        const audioSrc = await loadAudioFile(w.audio);
        audio[w.audio] = await audioUrlToBase64(audioSrc);
      } catch {
        // skip missing audio
      }
    }
  ...
  const backup: BackupFile = {
    version: 2, // Updated version for audio support
...
export async function restoreBackup(file: File) {
  ...
  // Restore audio files (if available in backup)
  if (backup.audio) {
    for (const [audioId, base64] of Object.entries(backup.audio)) {
      const file = base64ToAudioFile(base64);
...
function audioUrlToBase64(src: string): Promise<string> {
  return new Promise((resolve, reject) => {
    fetch(src)
...
function base64ToAudioFile(base64: string): File {
  ...
  const u8arr = new Uint8Array(n);
  while (n--) u8arr[n] = bstr.charCodeAt(n);
  return new File([u8arr], "audio.webm", { type: mime });
}
